"""
MCM 2026 Problem B - 第二问：最终版本 v12
修复：
1. 蒙特卡洛分布更集中（修正参数错误）
2. 3D曲面：单边高时cost/time明显高于两边都高
"""
...
class TransportCostTimeModel:
    ...
    def calculate_cost(self, a, p, q):
        ...
        if self.penalty_mode == 'strong':
            ...
            # ===== 关键改进1：添加协同惩罚因子 =====
            # 当p和q都接近1时，协同效应好，惩罚减少
            # 当p或q有一个很低时，协同效应差，额外惩罚
            synergy_factor = p_safe * q_safe  # 范围[0.04, 1.0]
            
            # 协同惩罚：synergy_factor越小，惩罚越大
            # 当p=1,q=1时，synergy_penalty=1（无额外惩罚）
            # 当p=1,q=0.5时，synergy_penalty=1.5（额外50%惩罚）
            # 当p=0.5,q=0.5时，synergy_penalty=2.0（额外100%惩罚）
            synergy_penalty = 1.0 / (0.3 + 0.7 * synergy_factor)  # 范围[1.0, 3.3]
            ...
            synergy_penalty = 1.0  # 蒙特卡洛不使用协同惩罚
...
#         # ===== 修正：大幅降低标准差 =====
#         # 直接设定极小的标准差，而不是基于原始标准差计算
#         p_target_std = 0.015  # 固定值：约1.8% CV
#         q_target_std = 0.008  # 固定值：约1.1% CV
        
#         # 严格的采样范围
#         p_lower = max(0.78, base_p - 2.0 * p_target_std)
#         p_upper = min(0.90, base_p + 2.0 * p_target_std)
        
#         q_lower = max(0.68, base_q - 2.0 * q_target_std)
#         q_upper = min(0.72, base_q + 2.0 * q_target_std)
...
#         # 温和的异常值过滤
#         def mild_filter(data, factor=2.5):
#             if len(data) < 10:
...
#         times_filtered, time_mask = mild_filter(times)
...
class Visualizer:
    ...
    def plot_3d_optimization_surface(model, p_range, q_range, resolution=30, smooth_sigma=0.9):
        ...
        print(f"  【新特性】单边高(p=1,q=0.5)的cost/time将明显高于两边都高(p=1,q=1)")
        ...
        cost_q_high = Costs[idx_q1, idx_p05]
        
        print(f"\n  验证协同效应:")
        print(f"  - p=1.0, q=1.0: Cost = {cost_both_high:.1f} B$")
        print(f"  - p=1.0, q=0.5: Cost = {cost_p_high:.1f} B$ (比两边都高多 {(cost_p_high/cost_both_high-1)*100:.1f}%)")
        print(f"  - p=0.5, q=1.0: Cost = {cost_q_high:.1f} B$ (比两边都高多 {(cost_q_high/cost_both_high-1)*100:.1f}%)")
        ...
        ax2.set_title('Completion Time Surface\n(With Synergy Effect)', fontsize=12, pad=20, fontweight='bold')
...
def main():
    print("=" * 80)
    print("MCM 2026 Problem B - 第二问：最终版本 v12")
    print("=" * 80)
    print("\n【改进】")
    print("1. 蒙特卡洛：修正参数，真正实现集中分布")
    print("2. 3D曲面：添加协同效应，单边高时cost/time明显高于两边都高")
    ...
    fig1.savefig('3d_surface_v12.png', dpi=200, bbox_inches='tight')
    print("     3d_surface_v12.png")
    
    print("[2/4] 帕累托前沿...")
    fig2 = visualizer.plot_pareto_front(pareto_points, all_a, all_times, all_costs, p, q)
    fig2.savefig('pareto_front_v12.png', dpi=200, bbox_inches='tight')
    print("     pareto_front_v12.png")
    
    print("[3/4] 最优权重...")
    fig3 = visualizer.plot_optimal_a_vs_weights(weight_results, p, q)
    fig3.savefig('optimal_weights_v12.png', dpi=200, bbox_inches='tight')
    print("     optimal_weights_v12.png")
    
    print("[4/4] 蒙特卡洛（集中）...")
    fig4 = visualizer.plot_monte_carlo_results(mc_results, 0.5)
    fig4.savefig('monte_carlo_v12.png', dpi=200, bbox_inches='tight')
    print("     monte_carlo_v12.png")
