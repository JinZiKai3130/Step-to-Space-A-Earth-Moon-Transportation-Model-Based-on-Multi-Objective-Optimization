"""
MCM 2026 Problem B - 第二问：最终版本 v10
修复：确保所有图表都能生成
"""
...
class MonteCarloSimulator:
    ...
    def run_simulation(self, a, num_simulations=1500, selected_sites=None):
        ...
        # ===== 修复：使用更稳健的���数 =====
        p_target_std = base_p * 0.05
        q_target_std = base_q * 0.017
        
        p_lower = max(0.70, base_p - 2.0 * p_target_std)
        p_upper = min(0.95, base_p + 2.0 * p_target_std)
        
        q_lower = max(0.65, base_q - 2.0 * q_target_std)
        q_upper = min(0.75, base_q + 2.0 * q_target_std)
        ...
        for i in range(num_simulations * 2):  # 最多尝试2倍次数
        ...
        # 温和的异常值过滤
        def safe_filter(data, factor=3.0):
            if len(data) < 10:
...
def main():
    print("=" * 80)
    print("MCM 2026 Problem B - 第二问：最终版本 v10 (修复版)")
    print("=" * 80)
    ...
    fig1.savefig('3d_surface_v10.png', dpi=200, bbox_inches='tight')
    print("     3d_surface_v10.png")
    
    print("[2/4] 帕累托前沿...")
    fig2 = visualizer.plot_pareto_front(pareto_points, all_a, all_times, all_costs, p, q)
    fig2.savefig('pareto_front_v10.png', dpi=200, bbox_inches='tight')
    print("     pareto_front_v10.png")
    
    print("[3/4] 最优权重...")
    fig3 = visualizer.plot_optimal_a_vs_weights(weight_results, p, q)
    fig3.savefig('optimal_weights_v10.png', dpi=200, bbox_inches='tight')
    print("     optimal_weights_v10.png")
    
    print("[4/4] 蒙特卡洛...")
    fig4 = visualizer.plot_monte_carlo_results(mc_results, 0.5)
    fig4.savefig('monte_carlo_v10.png', dpi=200, bbox_inches='tight')
    print("monte_carlo_v10.png")
