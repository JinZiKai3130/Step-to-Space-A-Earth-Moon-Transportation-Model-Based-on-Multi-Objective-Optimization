"""
MCM 2026 Problem B - 第二问：最终版本 v11
优化：让蒙特卡洛分布更集中，减少两端散布
"""
...
class MonteCarloSimulator:
    ...
    def run_simulation(self, a, num_simulations=1500, selected_sites=None):
        ...
        try:
            ...
            print(f"  错误: {e}")
            base_p, p_std = 0.83, 0.04
            base_q, q_std = 0.70, 0.012
        
        print(f"  基准 p = {base_p:.4f} ± {p_std:.4f}")
        print(f"  基准 q = {base_q:.4f} ± {q_std:.4f}")
        
        # ===== 关键调整1: 大幅减小标准差，让分布更集中 =====
        # 目标CV: 时间~1%, 成本~0.5%, p~3%, q~1%
        p_target_std = base_p * 0.03   # p的CV目标: 3% (原5%)
        q_target_std = base_q * 0.01   # q的CV目标: 1% (原1.7%)
        
        # ===== 关键调整2: 更严格的采样范围 =====
        p_lower = max(0.75, base_p - 1.5 * p_target_std)
        p_upper = min(0.92, base_p + 1.5 * p_target_std)
        
        q_lower = max(0.68, base_q - 1.5 * q_target_std)
        q_upper = min(0.72, base_q + 1.5 * q_target_std)
        
        print(f"  采样范围: p ∈ [{p_lower:.3f}, {p_upper:.3f}] (更窄)")
        print(f"  采样范围: q ∈ [{q_lower:.3f}, {q_upper:.3f}] (更窄)")
        ...
        # ===== 关键调整4: 更严格的异常值过滤 =====
        def tight_filter(data, factor=2.0):  # 从3.0降到2.0
            if len(data) < 10:
        ...
        # ===== 关键调整5: 两次过滤，进一步收紧分布 =====
        # 第一次：基于时间过滤
        times_filtered1, time_mask1 = tight_filter(times, factor=2.0)
        costs_temp1 = costs[time_mask1]
        p_temp1 = p_values[time_mask1]
        q_temp1 = q_values[time_mask1]
        
        # 第二次：基于成本再过滤
        costs_filtered2, cost_mask2 = tight_filter(costs_temp1, factor=2.0)
        times_filtered = times_filtered1[cost_mask2]
        costs_filtered = costs_filtered2
        p_filtered = p_temp1[cost_mask2]
        q_filtered = q_temp1[cost_mask2]
        
        print(f"   双重过滤后样本数: {len(times_filtered)}")
...
class Visualizer:
    ...
    def plot_monte_carlo_results(mc_results, a):
        ...
        # 时间分布 - 增加bins数量以显示更细致的集中效果
        axes[0, 0].hist(mc_results['times'], bins=80, density=True, alpha=0.75, 
        ...
        axes[0, 0].set_title('Time Distribution (Concentrated)', fontsize=12, fontweight='bold')
        axes[0, 0].legend(fontsize=10)
        axes[0, 0].grid(True, alpha=0.25, linestyle='--')
        
        # 成本分布
        axes[0, 1].hist(mc_results['costs']/1e9, bins=80, density=True, alpha=0.75, 
        ...
        axes[0, 1].set_title('Cost Distribution (Concentrated)', fontsize=12, fontweight='bold')
        axes[0, 1].legend(fontsize=10)
        axes[0, 1].grid(True, alpha=0.25, linestyle='--')
        
        # p分布
        axes[1, 0].hist(mc_results['p_values'], bins=80, density=True, alpha=0.75, 
        ...
        axes[1, 0].set_title('Elevator Stability (Concentrated)', fontsize=12, fontweight='bold')
        axes[1, 0].legend(fontsize=10)
        axes[1, 0].grid(True, alpha=0.25, linestyle='--')
        
        # q分布
        axes[1, 1].hist(mc_results['q_values'], bins=80, density=True, alpha=0.75, 
        ...
        axes[1, 1].set_title('Rocket Stability (Concentrated)', fontsize=12, fontweight='bold')
...
def main():
    print("=" * 80)
    print("MCM 2026 Problem B - 第二问：最终版本 v11")
    print("=" * 80)
    print("\n【优化】蒙特卡洛分布更集中，两端散布更少")
    ...
    print(f"\n  分布特征:")
    print(f"  - p范围: [{mc_results['p_values'].min():.3f}, {mc_results['p_values'].max():.3f}]")
    print(f"  - q范围: [{mc_results['q_values'].min():.3f}, {mc_results['q_values'].max():.3f}]")
    print(f"  - 时间范围: [{mc_results['times'].min():.1f}, {mc_results['times'].max():.1f}] 年")
    print(f"  - 成本范围: [{mc_results['costs'].min()/1e9:.1f}, {mc_results['costs'].max()/1e9:.1f}] B$")
    ...
    fig1.savefig('3d_surface_v11.png', dpi=200, bbox_inches='tight')
    print("     3d_surface_v11.png")
    
    print("[2/4] 帕累托前沿...")
    fig2 = visualizer.plot_pareto_front(pareto_points, all_a, all_times, all_costs, p, q)
    fig2.savefig('pareto_front_v11.png', dpi=200, bbox_inches='tight')
    print("     pareto_front_v11.png")
    
    print("[3/4] 最优权重...")
    fig3 = visualizer.plot_optimal_a_vs_weights(weight_results, p, q)
    fig3.savefig('optimal_weights_v11.png', dpi=200, bbox_inches='tight')
    print("     optimal_weights_v11.png")
    
    print("[4/4] 蒙特卡洛（更集中）...")
    fig4 = visualizer.plot_monte_carlo_results(mc_results, 0.5)
    fig4.savefig('monte_carlo_v11.png', dpi=200, bbox_inches='tight')
    print("     monte_carlo_v11.png")
    
    print("\n" + "=" * 80)
    print("完成！蒙特卡洛分布已优化为更集中（两端更少）")
    print("=" * 80)
