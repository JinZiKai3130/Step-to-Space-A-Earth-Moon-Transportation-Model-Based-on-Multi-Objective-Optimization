"""
MCM 2026 Problem B - 第二问：混合版本
策略：保持v7的3D曲面效果 + 回退到v4的蒙特卡洛参数
"""
...
class MonteCarloSimulator:
    ...
    def run_simulation(self, a, num_simulations=1500, selected_sites=None):
        ...
        # ===== 回退到v4的采样控制（较宽松，允许更大散度） =====
        p_lower = max(0.35, base_p - 3.0 * p_std)  # v4: 3.0倍标准差
        p_upper = min(0.98, base_p + 3.0 * p_std)
        
        q_lower = max(0.50, base_q - 3.0 * q_std)
        q_upper = min(0.98, base_q + 3.0 * q_std)
        ...
        while valid_samples < num_simulations and iterations < max_iterations:
            ...
            # ===== v4设置：不缩小标准差，允许完整的随机性 =====
            p = np.clip(np.random.normal(base_p, p_std), p_lower, p_upper)
            q = np.clip(np.random.normal(base_q, q_std), q_lower, q_upper)
        ...
        # ===== v4设置：更宽松的IQR过滤（factor=3.0） =====
        if len(times) > 10:
            def filter_outliers(data, factor=3.0):  # v4: factor=3.0（更宽松）
                if len(data) < 4:
...
class Visualizer:
    ...
    def plot_monte_carlo_results(mc_results, a):
        ...
        plt.suptitle(f'Monte Carlo Simulation Results (a={a:.2f}, N={len(mc_results["times"])})\n' + 
                    '✓ V4 Parameters | Larger Variance for Better Distribution', 
...
def main():
    print("=" * 80)
    print("MCM 2026 Problem B - 第二问：混合最终版本 (v8)")
    print("=" * 80)
    print("\n【策略】")
    print(" 3D曲面：保持v7参数（强惩罚模式）")
    print(" 蒙特卡洛：回退到v4参数（较大散度）")
    ...
    fig1.savefig('3d_surface_v8.png', dpi=200, bbox_inches='tight')
    print("     已保存: 3d_surface_v8.png")
    
    print("[2/4] 帕累托前沿...")
    fig2 = visualizer.plot_pareto_front(pareto_points, all_a, all_times, all_costs, p, q)
    fig2.savefig('pareto_front_v8.png', dpi=200, bbox_inches='tight')
    print("     已保存: pareto_front_v8.png")
    
    print("[3/4] 最优权重...")
    fig3 = visualizer.plot_optimal_a_vs_weights(weight_results, p, q)
    fig3.savefig('optimal_weights_v8.png', dpi=200, bbox_inches='tight')
    print("     已保存: optimal_weights_v8.png")
    
    print("[4/4] 蒙特卡洛 (v4参数)...")
    fig4 = visualizer.plot_monte_carlo_results(mc_results, 0.5)
    fig4.savefig('monte_carlo_v8.png', dpi=200, bbox_inches='tight')
    print("     已保存: monte_carlo_v8.png")
    
    print("\n" + "=" * 80)
    print("完成！3D曲面保持v7效果，蒙特卡洛恢复v4散度")
    print("=" * 80)
